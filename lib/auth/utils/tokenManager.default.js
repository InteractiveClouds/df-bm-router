...
function Constructor ( o ) {
    ...
    this.isInited   =
        ...
        .fail(function(error){
            log.error('Token manager initializing error: ', error);
        });
}
...
Constructor.fn.formatAnswer = function ( token, user ) {

    var that = this;

    return {
        token : that.crypt.encrypt(JSON.stringify({
                id        : token.id,
                secret    : token.secret,
...
Constructor.fn.create = function (tenantid, userid, appid, session) {
    ...
    return Q.all([
        ...
        this.users.get(tenantid, userid)
    ])
    .spread(function(tenant, user){
        ...
        return Q.when(that.composeToken({
            ...
            .then(function (id) {
                ...
                return Q(that.formatAnswer(token, user));
            });
        });
    });
};
