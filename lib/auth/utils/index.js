...
var confFilePath = path.resolve(__dirname, '../.auth.conf'),
...
function createStoreCredentialsPasswordFile () {
    return Q.all([
            random(64),
            random(128)
    ]).spread(function ( pass, nonce ) {
        return QFS.write(confFilePath, 'exports.storagePass = \'' + pass + '\';\nexports.nonce = \'' + nonce + '\';\n', {mode:0400});
    })
    .then(
...
function initCheck () {
    ...
    .then(function ( docs ) {
        ...
        if ( !storagePassHash ) {
            log.fatal('Can not found .auth.conf.');
        }
